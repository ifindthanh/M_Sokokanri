		...
		<form action="tat-ca" method="POST">
			<div class="col-sm-12 row">
				<div class="col-xs-12 row">
					<label class="col-xs-2 right_align top_margin_5">Tên nhóm loại cây: </label>
					<div class="col-xs-4">
					...
					<tbody>
						...
						<c:forEach var="item" items="${allTrees}" varStatus="status">
							<tr class="table-row" tree_id="${item.id }">
								<td>
									<chkbox2:chbox item="${item.id }" selectedItems="${selectedItems}" action="../nhom-cay"/>
								</td>
								<td>
										${item.name}
								</td>
...
<script>
	...
	function deleteTrees() {
		if ($('.order_id:checkbox:checked').length == 0) {
			alert("Vui lòng chọn nhóm loại cây.");
			return;
		}
		var check = confirm("Bạn có chắc muốn xóa nhóm loại cây đã chọn?");
		if (check) {
			$('#ajax-overlay').show();
			$.ajax({
				type : "GET",
				...
				success : function(result) {
					...
					alert("Xóa nhóm loại cây thành công");
					window.location.href = window.location.href.split("#")[0];
				},
				error : function() {
					alert("Xóa nhóm loại cây thất bại, vui lòng liên hệ quản trị viên");
					$('#ajax-overlay').hide();
				}
			});
		}
