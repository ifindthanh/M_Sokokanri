...
<%@ taglib prefix="checkbox" uri="/WEB-INF/taglibs/exportedHeaderCheckbox.tld"%>
...
<body>
					...
					<div class="main_container">
						<div class="">
							<form id="exportingItems"
								...
								action="items" method="POST" enctype="multipart/form-data">
								<div class="row">
											...
											<div class="x_content">
												...
												<div class="row">
													<div class="form-group">
														...
														<label class="control-label col-xs-1 left-group"
															for="first-name">出荷日 </label>
														<div class="col-xs-2">
															<input name="shukkaDate" type="text"
																class="form-control has-feedback-left testDate"
																style="ime-mode: disabled" id="shukkaDate"
																placeholder="YYYY/MM/DD"
																value="${searchCondition.getShukkaDate()}"> <span
													...
													<div class="form-group pull-center"
														...
														<div class="col-xs-5" style="text-align: left">
															<a class="btn left" href=""
																onclick="checkSelectedItems(event)"
																style="margin-left: 5px; text-decoration: underline !important;">
																<i class="fa fa-download" aria-hidden="true"></i> QJ2をエクスポート
															</a>
										...
										<div class="x_panel table-panel">
											<div class="x_content">
															...
															<tbody>
																...
																<c:forEach var="item" items="${exportItems}">
																	...
																	<tr class="pointer <%=class_ex%>" style="height: 30px;"
																		role="row">
																		<td>${item.getShukkaHeadBean().getId().getSojoNo()}</td>
																		<td><div style="width:80px">
																			<fmt:formatDate value="${item.getShukkaHeadBean().getSukkaYoteibi()}" pattern="yyyy/MM/dd" />
																		</div></td>
																		<td colspan="10" style="text-align:left" >${item.getShukkaHeadBean().getJyuoka()}</td>
																	...
																	<c:forEach var="itemBody"
																		...
																		<tr class="pointer <%=class_ex%>"
																			...
																			<td>${itemBody.yobi}</td>
																			<td>${itemBody.szNo}</td>
																			<td><fmt:formatDate value="${itemBody.shukkaDate}" pattern="yyyy/MM/dd" /></td>
																		</tr>
																	</c:forEach>
																</c:forEach>
															</tbody>
												...
												<div class="div-bottom">
													...
													<tag:paginate offset="${offset}" count="${count}"
														steps="${maxResult}"
														uri="${pageContext.request.contextPath}/exported/items"
	...
	function getMyTableBodyHeight()
    {
        ...
        height -= 144; // height of search input area
    ...
    $(window).resize(function () {
		...
		function checkSelectedItems(e) {
			...
			$.dialogbox({
				type : 'msg',
				title : '確認',
				content : "QJ2をエクスポートします、宜しいでしょうか？",
				closeBtn : true,
				btn : [ 'OK', 'キャンセル' ],
				call : [ function() {
